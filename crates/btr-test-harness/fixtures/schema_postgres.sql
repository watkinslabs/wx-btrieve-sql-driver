-- Postgres fixture for the btr-test-harness.
--
-- Same shape as schema.sql / schema_sqlite.sql. CHAR(N) is preserved so
-- comparisons work like MSSQL (Postgres CHAR pads on either side too,
-- unlike its TEXT/VARCHAR). The recnum column is BIGINT GENERATED ALWAYS
-- AS IDENTITY to match dialect.identity_column().
...
CREATE TABLE "TEST_CUST" (
    "MDS_RECNUM"  BIGINT GENERATED ALWAYS AS IDENTITY PRIMARY KEY,
...
CREATE TABLE "TEST_AUTOINC" (
    "AUTO_ID"  BIGINT GENERATED ALWAYS AS IDENTITY PRIMARY KEY,
...
CREATE TABLE "TEST_TYPES" (
    ...
    "LOG_VAL"    SMALLINT NOT NULL DEFAULT 0,
    "DATE_VAL"   DATE
);
